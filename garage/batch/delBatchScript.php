...
$stmt = $link->prepare("DELETE FROM batch WHERE id=?;");
...
<body class="text-center m-md-5 p-md-5 p-3 text-light bg-dark">
    ...
    <p><?php echo !$stmt->error ? '<i class="pe-2 bi bi-check-circle-fill text-success"></i>Várka byla odstraněna' : ('<i class="pe-2 bi bi-exclamation-circle-fill text-danger"></i>' . $stmt->error) ?></p>
